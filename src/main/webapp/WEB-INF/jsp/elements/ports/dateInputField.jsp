...
<div class="form_field ${entryRow.index != 0 && isFileBlock == false && portRow.index == 0 ? "contains_repeat_control" : ""}">
	...
	<c:choose>
		<c:when test="${port.datePrecision.name == 'month'}">
			<form:input cssClass="monthpicker ${port.blankDefaultDate == true ? 'blankDefault' : ''}" path="elements[${elementRow.index}].entries[${entryRow.index}].fields[${portRow.index}].value" title="${port.usage}" placeholder="${port.usage}" />
		</c:when>
		<c:when test="${port.datePrecision.name == 'day'}">
			<form:input cssClass="datepicker ${port.blankDefaultDate == true ? 'blankDefault' : ''}" path="elements[${elementRow.index}].entries[${entryRow.index}].fields[${portRow.index}].value" title="${port.usage}" placeholder="${port.usage}" />
		</c:when>
